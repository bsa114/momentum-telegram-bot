...
def apply_fill_results(
    ...
    for item in filled_rows:
        ...
        if not isinstance(row, int) or row < 1:
            # Некорректный номер строки (0, отрицательное число, не число вовсе) —
            # такое может прийти, если модель ошиблась в JSON-ответе, либо
            # LLM-фолбэк разбора свободного текста некорректно сопоставил
            # вариант. Пропускаем эту запись, не давая упасть всей функции.
